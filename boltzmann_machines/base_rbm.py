from __future__ import print_function # to use tf.print in python 2.7
import numpy as np
...
from tf_model import run_in_tf_session
from utils import (make_list_from, batch_iter, epoch_iter,
                                      write_during_training)
from testing import assert_len, assert_shape
...
class BaseRBM(EnergyBasedModel):
    ...
    def __init__(self,
                 ...
                 learning_rate=0.01, momentum=0.9, max_epoch=10, batch_size=10, l2=0.0, # changed l2 from 1e-4 to 0
                 ...
                 display_filters=10, display_hidden_activations=0, v_shape=(28, 28),
        ...
        self._q_means = None
        # added by Ziwei Li
        self._ind_filter = None
        self._ind_center = None
        self._dW_temp = None
        self._dW1_temp = None
        self._dW0_temp = None
    ...
    def _make_filters(self):
        ...
        with tf.name_scope('initialize_filter_pos'):
            
            ind_filter, ind_center = self._cal_filter_positions()
            
            self._ind_filter = tf.Variable(ind_filter, dtype=tf.int32, name='ind_filter')
            self._ind_center = tf.Variable(ind_center, dtype=tf.int32, name='ind_center')
    ...
    def _make_vars(self):
        ...
        with tf.name_scope('weights'):
            ...
            '''
            # random initialization of W_init
            if hasattr(self.W_init, '__iter__'):
            ...
            dw1 = tf.random_normal([], 1)
            dw0 = tf.random_normal([], 1)
            # set non-filter areas to zero
            dW1_temp = tf.scatter_nd(temp_ind_1, dw1*tf.ones(temp_ind_1.shape[0]), [self.n_visible, self.n_hidden])
            dW0_temp = tf.scatter_nd(temp_ind_2, dw0*tf.ones(temp_ind_2.shape[0]), [self.n_visible, self.n_hidden])
            
            W_init = dW1_temp + dW0_temp
        ...
        with tf.name_scope('init_temp_weights'):
            Zeros_init = tf.zeros([self.n_visible, self.n_hidden])
            self._dW_temp = tf.Variable(Zeros_init, dtype=self._tf_dtype, name='dW_temp')
            self._dW1_temp = tf.Variable(Zeros_init, dtype=self._tf_dtype, name='dW1_temp')
            self._dW0_temp = tf.Variable(Zeros_init, dtype=self._tf_dtype, name='dW0_temp')

        #with tf.name_scope('initialize_W'):
        # visualize filters
        if self.display_filters:
            with tf.name_scope('filters_visualization'):
                ...
                # the following line does not work under eager mode, commented
    ...
    def _cal_filter_positions(self):

        # set all positions other than the filter to be zero. 
        # for the first try, the filter is designed to be:
        #   |w1|
        #|w1|w0|w1|
        #   |w1|

        L_visible_row = np.sqrt(self.n_visible).astype('int32')
        L_visible_col = L_visible_row
        j_indices_0 = np.array(range(self.n_hidden));
        j_indices = j_indices_0 / (L_visible_col/2) % 2 + 2 * (j_indices_0 + 1) - 2 # grid of 45-degree rotation
        [row_in_visible, col_in_visible] = np.unravel_index(j_indices, (L_visible_row, L_visible_col))
        
        ind_filter = np.zeros([self.n_hidden, 4], dtype='int32')
        ind_center = np.zeros([self.n_hidden, 1], dtype='int32')
        for j in range(len(j_indices)):
            # center point
            loc_center = (row_in_visible[j], col_in_visible[j])
            # ajacent four points
            temp = np.array([[row_in_visible[j] + 1, col_in_visible[j]], 
                             [row_in_visible[j] - 1, col_in_visible[j]], 
                             [row_in_visible[j], col_in_visible[j] + 1], 
                             [row_in_visible[j], col_in_visible[j] - 1]])
            temp_ind = temp % np.tile([L_visible_row, L_visible_col], (temp.shape[0], 1))
            ind_filter[j, :] = np.ravel_multi_index(temp_ind.T, (L_visible_row, L_visible_col))
            ind_center[j, 0] = np.ravel_multi_index(loc_center, (L_visible_row, L_visible_col))
        
        return ind_filter, ind_center   
    ...
    def _make_train_op(self):
        ...
        with tf.name_scope('grads_estimates'):
            ...
            with tf.variable_scope('dW'):
                ...
                with tf.name_scope('filter_update'):
                    ...
                    dW1_temp = tf.scatter_nd(temp_ind_1, temp_values, tf.shape(dW))
                with tf.name_scope('center_update'):
                    ...
                    dW0_temp = tf.scatter_nd(temp_ind_2, temp_values_2, tf.shape(dW))
        ...
        with tf.name_scope('pseudo_loglik'):
            ...
            m = tf.SparseTensor(indices=tf.cast(ind, dtype='int64'),
                                 values=tf.ones_like(pll_rand, dtype=self._tf_dtype),
                                 dense_shape=tf.cast(tf.shape(x_), dtype='int64'))
    ...
    def _fit(self, X, X_val=None, *args, **kwargs):
        ...
        # collect metrics to measure goodness of fit for training data
        self._train_metrics_map = {}
    ...
    def gather_indices(self, ind_filter, filter_size):
        with tf.name_scope('gather_indices'):
            # for simplicity, filter_size is a regular numpy constant
            W_shape = [self.n_visible, self.n_hidden]

            #temp_range = tf.Variable(tf.zeros([filter_size, self._n_hidden], dtype=tf.int32), 
            #            name = 'temp_range', dtype=tf.int32)
            temp_range = np.zeros([filter_size, self.n_hidden]).astype('int32')
                    #collections=[tf.GraphKeys.LOCAL_VARIABLES], dtype=tf.int32)
        
            # get layer indices (indices wrt. hidden units)
            #temp_ind_0 = np.array(range(W_shape[1])*4).reshape(4, W_shape[1]).T.reshape(4*W_shape[1], 1)
        
            temp_range_0 = range(self.n_hidden)

            for i in range(filter_size):
                temp_range[i, :] = temp_range_0
            
            temp_ind_0 = tf.reshape(
                            tf.transpose(
                                tf.reshape(temp_range, (filter_size, self.n_hidden))
                                ), (tf.multiply(filter_size, self.n_hidden), 1))
            #print_op = tf.print(temp_ind_0)
            #with tf.control_dependencies([print_op]):
            temp_ind_1 = tf.concat([
                    tf.reshape(ind_filter, [filter_size*self.n_hidden, 1]),
                    temp_ind_0], 1)
            return temp_ind_1
